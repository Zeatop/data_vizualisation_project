...
Promise.all(files.map((url, i) => i < 2 ? d3.json(url) : d3.csv(url)))
    .then(([us, cities, monthData, weekData, lateData]) => {
        ...
        gCities.selectAll("circle")
            .data(cities.features.filter(d => state.lookup[d.properties.city])) // Garde seulement ceux avec coords
            .enter().append("circle")
            .attr("class", "city")
            .attr("transform", d => `translate(${projection(d.geometry.coordinates)})`)
            .attr("r", d => sizeScale(cityCounts[d.properties.city] || 0))
            .each(function(d) { d.baseR = sizeScale(cityCounts[d.properties.city] || 0); }) // Stocke rayon de base
            .on("click", d => selectCity(d.properties.city))
            .on("mouseover", d => showTooltip(d.properties.city, d3.event.pageX, d3.event.pageY));
...
function updateMap() {
    ...
    routes.merge(enterRoutes)
        ...
        .style("stroke-width", d => state.selectedCity ? 2 : 0.5) // Plus épais si focus
        .on("mouseover", function(d) {
            d3.select(this).style("stroke-width", 4).raise();
            const latePct = d.flight_number > 0 ? ((d.is_late/d.flight_number)*100).toFixed(1) : 0;
            showTooltip(`${d.origin_city} → ${d.dest_city}<br>Vols: ${d.flight_number}<br>Retard: ${latePct}%`, d3.event.pageX, d3.event.pageY);
        })
